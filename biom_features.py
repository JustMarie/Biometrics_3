...
def Scale(src, m, n):
    #M, N = src.shape
    small_img = cv.resize(src, (n, m), interpolation=cv.INTER_AREA)
...
def DFT(img, p):
    ...
    FXcos = np.matmul(F_pM_Cos, img)
    FXsin = np.matmul(F_pM_Sin, img)

    Creal = np.matmul(FXcos, F_Np_Cos) - np.matmul(FXsin, F_Np_Sin)
    Cimage = np.matmul(FXcos, F_Np_Sin) + np.matmul(FXsin, F_Np_Cos)
...
def DCT(img, p):
    ...
    C = np.matmul(np.matmul(T_pM, img), T_Np)
    return C
